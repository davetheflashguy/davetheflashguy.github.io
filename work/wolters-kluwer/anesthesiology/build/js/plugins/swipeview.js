...
var SwipeView = (function (window, document) {
	var dummyStyle = document.createElement('div').style,
		...
		SwipeView = function (el, options) {
			...
			div.style.cssText = 'position:relative;top:0;' + cssVendor + 'transition-duration:0;' + cssVendor + 'transform:translateZ(0);' + cssVendor + 'transition-timing-function:ease-out';
			...
			for (i=-1; i<2; i++) {
				...
				div.style.cssText = cssVendor + 'transform:translateZ(0);position:absolute;top:0;left:' + i*100 + '%';
...
/*			if (!hasTouch) {
		...
		refreshSize: function () {
			...
			this.snapThreshold = this.options.snapThreshold === null ?
				Math.round(this.pageWidth * 0.05) :
...
/*			if (newX > 0 || newX < this.maxX) {
			...
			console.log('moving');
			$("#wrapper").trigger('horizontal_move_in_progress');
		},
		...
		__end: function (e) {
			...
			this.__checkPosition();
			$("#wrapper").trigger('horizontal_move_complete');
		},
